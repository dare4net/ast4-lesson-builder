...
describe('expandable missions and achievements', () => {
    ...
    it('locks catalog editing behind the env-auth superadmin console', () => {
        ...
        expect(read('app/superadmin/page.tsx')).toContain('Pick a lesson first')
        expect(read('app/superadmin/page.tsx')).not.toContain('placeholder="l3-complete-three-lessons"')
        expect(read('app/superadmin/page.tsx')).not.toContain('id: isNewMission')
        expect(read('app/superadmin/page.tsx')).toContain('createMission(payload)')
        expect(read('app/superadmin/page.tsx')).toContain('createAchievement(payload)')
    ...
    it('hides type/mode and names the block when a specific block is chosen', () => {
        ...
        expect(page).toContain('describeMissionRecipe')
        expect(page).toContain('describeAchievementRecipe')

        expect(page).toContain('canUsePerfectAttempt')
        expect(page).toContain('isScoredCatalogType')
