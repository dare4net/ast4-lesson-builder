        ...
        <div className="w-full h-full flex-1 flex flex-col justify-center px-4 sm:px-6 py-4 relative min-h-0 overflow-hidden text-slate-900">
            ...
            {hint && (
                <div className="mb-4 shrink-0 flex flex-col items-center">
                    ...
                    {showHint && (
                        <div className="mt-2 text-xs font-bold text-slate-700 bg-sky-50/60 p-2.5 rounded-2xl border border-sky-200 animate-in fade-in flex items-center justify-between gap-3 max-w-md w-full">
                            <span>💡 {hint}</span>
                            <button
                                type="button"
                                onClick={handleSpeakHint}
                                className="flex items-center gap-1 px-2 py-1 rounded-lg bg-sky-100 hover:bg-sky-200 text-sky-700 text-[10px] font-black transition-all border border-sky-200 shrink-0 cursor-pointer"
